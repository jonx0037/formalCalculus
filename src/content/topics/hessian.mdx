...
| Topic | Connection |
|-------|-----------|
| [The Derivative](/topics/derivative) | The single-variable $f''(a)$ is the $1 \times 1$ Hessian. The second derivative test generalizes to eigenvalue analysis. |
| [Taylor's Theorem](/topics/mean-value-taylor) | The single-variable Taylor expansion $f(a+h) = f(a) + f'h + \frac{1}{2}f''h^2 + O(h^3)$ generalizes to the multivariable second-order expansion with the quadratic form $h^T H_f h$. |
| [Partial Derivatives & the Gradient](/topics/gradient) | The gradient provides first-order information (slope). The Hessian provides second-order information (curvature). At critical points, the gradient vanishes and the Hessian determines the local structure. |
| [The Jacobian & Multivariate Chain Rule](/topics/jacobian) | The Hessian *is* the Jacobian of the gradient: $H_f = J(\nabla f)$. The chain rule enables computing Hessians of composed functions. |
