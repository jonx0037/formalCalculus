...
- **Mean Value Theorem & Taylor Expansion** *(coming soon)* — If $f$ is differentiable on $[a, b]$, there exists $c \in (a, b)$ with $f'(c) = \frac{f(b)-f(a)}{b-a}$. Taylor's theorem extends linear approximation to polynomial approximation of arbitrary degree.
