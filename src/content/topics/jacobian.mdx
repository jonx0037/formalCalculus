...
$\det J_f(a) = 0$ means the linear approximation $J_f(a)$ is not invertible — it "collapses" a neighborhood of $a$ into a lower-dimensional set. This is the boundary between the Inverse Function Theorem applying (nonzero determinant implies $f$ is locally invertible) and failing (zero determinant implies a critical value). **Inverse & Implicit Function Theorems** *(coming soon)* develops this connection.
</TheoremBlock>
