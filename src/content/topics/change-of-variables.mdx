...
- [The Riemann Integral & FTC](/topics/riemann-integral) — The substitution rule (§2) generalizes $u$-substitution to multiple variables.
- [The Jacobian & Multivariate Chain Rule](/topics/jacobian) — The Jacobian determinant, introduced as an abstract volume scaling factor in Topic 10, now does concrete computational work in every integral.
- [Multiple Integrals & Fubini's Theorem](/topics/multiple-integrals) — The painful disk integral (Example 7) is resolved by polar coordinates (§4). The Gaussian integral preview (§10) is proved in full (§5).
- [Improper Integrals & Special Functions](/topics/improper-integrals) — The Gaussian integral $\int e^{-x^2}\,dx = \sqrt{\pi}$, previewed in Topic 8, is proved here.

**Prerequisites used:**
- [Multiple Integrals & Fubini's Theorem](/topics/multiple-integrals) — Every integral in this topic is a multiple integral evaluated via Fubini.
- [Inverse & Implicit Function Theorems](/topics/inverse-implicit) — The IFT guarantees that $\varphi$ is a local diffeomorphism when $\det J_\varphi \neq 0$.

**What comes next within formalCalculus:**
- [**Surface Integrals & the Divergence Theorem**](/topics/surface-integrals) — Surface parameterization requires the Jacobian of the parameterization map; the surface area element involves the cross product of tangent vectors, which is the 2D analog of the Jacobian determinant.
...
- [Gradient Descent](https://formalml.com/topics/gradient-descent) → formalML — The reparameterization trick in variational inference is a change of variables applied to make gradients tractable.
- [Measure-Theoretic Probability](https://formalml.com/topics/measure-theoretic-probability) → formalML — The density transformation formula is the probabilistic form of the change of variables theorem.
- [Smooth Manifolds](https://formalml.com/topics/smooth-manifolds) → formalML — The partition-of-unity argument in the general proof connects to manifold integration.
- [Information Geometry](https://formalml.com/topics/information-geometry) → formalML — The Fisher information matrix transforms under reparameterization via the Jacobian.
