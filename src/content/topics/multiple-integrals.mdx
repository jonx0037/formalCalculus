...
This is computable (via $x = \sin\theta$ substitution) but awkward. In **polar coordinates** the same integral becomes $\int_0^{2\pi} \int_0^1 r \, dr \, d\theta = \pi$ — trivially. This is the motivation for the next topic: **Change of Variables** *(coming soon)*.
</TheoremBlock>
...
The [Gaussian integral](/topics/improper-integrals) $\int_{-\infty}^{\infty} e^{-x^2} dx = \sqrt{\pi}$ was proved by converting to polar coordinates — a *change of variables* that transforms the double integral into a simpler form. The general theory requires the [Jacobian determinant](/topics/jacobian) as a volume scaling factor. This is the subject of the next topic: **Change of Variables** *(coming soon)*.
...
This topic leads to:
- **Change of Variables** *(coming soon)* — the substitution rule for multiple integrals, with the Jacobian determinant as the volume scaling factor
