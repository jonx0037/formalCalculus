...
To integrate functions of two or more variables, we need to combine measures on different spaces into a product measure on the Cartesian product. The full theory — including Fubini's theorem, which lets us compute double integrals as iterated integrals — belongs to Topic 26 (The Lebesgue Integral). Here we lay the foundation.
...
- **The Lebesgue Integral** — builds the integral $\int f \, d\mu$ for non-negative measurable $f$ as the supremum of integrals of approximating simple functions, then extends to general measurable functions via $f = f^+ - f^-$. Proves the dominated convergence theorem and Fubini's theorem.
- **$L^p$ Spaces** — Banach spaces of measurable functions where $\|f\|_p = (\int |f|^p \, d\mu)^{1/p} < \infty$. Equivalence classes under a.e.-equality (the "null set" concept from Section 7 is essential).
