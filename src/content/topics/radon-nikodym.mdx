...
Then $f \geq 0$ everywhere (since $h \in [0, 1)$). Substituting back into $(\ast)$ with $g = (1 - h) \cdot \mathbf{1}_A / (1 - h) = \mathbf{1}_A$ — that is, working through the algebra of $(\ast\ast)$ — gives

$$\int_A (1 - h) \, d\nu = \int_A h \, d\mu = \int_A (1 - h) \cdot \frac{h}{1 - h} \, d\mu = \int_A (1 - h) f \, d\mu.$$

Dividing both sides by $(1 - h)$ (legitimate because $1 - h > 0$ on the relevant set) gives $\nu(A) = \int_A f \, d\mu$ for every measurable $A$. This is exactly the Radon-Nikodym identity: $f$ is the density we sought. Finiteness of $f$ in $L^1(\mu)$ follows from $\nu(\Omega) < \infty$, since $\int_\Omega f \, d\mu = \nu(\Omega) < \infty$.
...
- **Normed & Banach Spaces** — $L^p$ duality proved via Radon-Nikodym (now complete) is the canonical example of a dual-space characterization in functional analysis.
- **Inner Product & Hilbert Spaces** — Conditional expectation as $L^2$ projection onto a sub-$\sigma$-algebra is the prototypical Hilbert-space example. The full projection theorem and the spectral theorem build on the orthogonal-projection machinery developed here.
