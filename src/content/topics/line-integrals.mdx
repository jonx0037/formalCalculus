...
This is a scalar line integral (Definition 3) with the arc length element of the Fisher-Rao metric replacing the Euclidean one. Geodesics are curves that minimize this length integral — the [calculus of variations](/topics/calculus-of-variations) provides the Euler-Lagrange equation for finding them.
...
- [**First-Order ODEs & Existence Theorems**](/topics/first-order-odes) — Exact differential equations $M\,dx + N\,dy = 0$ are exact when $M_y = N_x$, the same criterion as conservative fields. The integrating factor technique corresponds to finding a potential function.
- [**Metric Spaces & Topology**](/topics/metric-spaces) — the topological vocabulary (open sets, continuity via preimages, homeomorphism, fundamental groups) underlying simply connected vs. non-simply-connected domains.
- [**Calculus of Variations**](/topics/calculus-of-variations) — Functionals $J[\gamma] = \int_a^b L(\gamma, \gamma', t)\,dt$ are line integrals over path space. Extremal paths satisfy the Euler-Lagrange equation.
