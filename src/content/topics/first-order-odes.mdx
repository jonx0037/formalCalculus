...
Consider $(y + t^2)\,dt + t\,dy = 0$. Check: $M_y = 1 \neq 2t = N_t$. Not exact. Try an integrating factor $\mu(t)$: we need $\frac{M_y - N_t}{N} = \frac{1 - 2t}{t}$ to depend only on $t$. It does: $\mu(t) = e^{\int (1/t - 2)\,dt} = te^{-2t}$. Multiplying through and verifying exactness yields the solution.
</TheoremBlock>
...
- **Linear Systems & Matrix Exponential** *(coming soon)* — systems $\mathbf{y}' = A\mathbf{y}$ generalize scalar linear equations; the matrix exponential $e^{At}$ extends the scalar solution $e^{at}$
- **Stability & Dynamical Systems** *(coming soon)* — phase portraits and Lyapunov stability classify the long-time behavior near equilibria
